...
void ACouchCatapult::SpawnBullet()
{
    ...
    if (MainProjectile)
    {
        ...
        MainProjectile->Initialize(SuggestedVelocity, GetCurrentPlayer(), PowerChargeComponent->TargetLocation);
    }
    ...
    if (ACouchProjectileEffect* ProjectileEffect = MainProjectile->GetEffectWithExecuteTime(ECouchProjectileExecuteTime::OnLaunch))
    {
        if (ACouchProjectileEffectTripleBullets* TripleBulletsEffect = Cast<ACouchProjectileEffectTripleBullets>(ProjectileEffect))
        {
            ...
            if (LeftProjectile)
            {
                ...
                LeftProjectile->Initialize(LeftVelocity, GetCurrentPlayer(), PowerChargeComponent->TargetLocation + FVector(-AngleOffset * UE::Geometry::Distance(StartLocation, PowerChargeComponent->TargetLocation), 0, 0)); // Utilise la vélocité recalculée
            }
            if (RightProjectile)
            {
                RightProjectile->InitCanonBall(AmmoActor);
                RightProjectile->Initialize(RightVelocity, GetCurrentPlayer(), PowerChargeComponent->TargetLocation + FVector(AngleOffset * UE::Geometry::Distance(StartLocation, PowerChargeComponent->TargetLocation), 0, 0)); // Utilise la vélocité recalculée
            }
        }
    }
